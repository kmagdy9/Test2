...
async function getApiData(){
  ...
  var https= await fetch(`https://www.themealdb.com/api/json/v1/1/list.php?i=list`, {method:"GET"});
  var response=await https.json();
  data=response.meals
  displayData();
  loadingScreen1();
  // for(var i=0;i<data.length;i++){
  //     meal=data[0].strIngredient;
  // }
...
function displayData(){
  ...
  for(var i=0;i<20;i++){
    ...
    cols+=`<div class="col-md-3 hamada ">
    ...
    <i class="fa-solid fa-drumstick-bite"></i>
      <div class="card-body">
        <h3 class="card-title text-center">${data[i].strIngredient}</h3>
...
function loadingScreen1(){
  ...
  $(function(){
    ...
    $(".ingredients .loader").fadeOut(2000,function(){
      $(".ingredients .loading").slideUp(2000 , function(){
